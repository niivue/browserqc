...
self.addEventListener(
  'message',
  function (event) {
    // BrowserQC patch: runInferenceWW is async but launched fire-and-forget upstream.
    // A rejection (model fetch / backend init) would become an unhandled worker
    // rejection that never reaches the host, so segment.ts runWorker() would hang
    // forever and wedge the single-flight queue. Catch it and emit the hard-failure
    // UI protocol (modalMessage contains "fail", which segment.ts recognises).
    Promise.resolve(
      runInferenceWW(event.data.opts, event.data.modelEntry, event.data.niftiHeader, event.data.niftiImage)
    ).catch((err) => {
      const msg = (err && err.message) || String(err)
      callbackUI(msg, -1, 'Segmentation failed: ' + msg)
    })
  },
  false
)
