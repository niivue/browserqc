...
**This is the only repo.** `neurolabusc/BrowserQC` (base `/BrowserQC/`) is archived; its last work (mindgrab migration, `browserqc.sh`) was ported here on 2026-09-09. Do not sync from it.
...
CLI reference the WASM call mirrors: `niimath --qc <t1> --seg <seg> --csf 3,4,11,12 --wm 1,5 [--erode 0|1] [--out qc.tsv]` (native niimath's `qc` branch adds `[--air <template>] [--json out.json]`, which the 1.3.3 WASM lacks) ([qc.c](../niimath/src/qc.c)). niimath's TSV names its air-free CNR `cnr_noair`; the air step (below) computes a real `cnr` that supersedes it (the panel falls back to `cnr_noair` if the air step was skipped).
...
**`browserqc.sh <in.nii[.gz]> <out.json>`** (repo root) — the native pipeline: `brainchop-16chan18cls` then `niimath --qc --air public/avg152T1.nii.gz --json`, both from PATH. niimath's `--air` (its `qc` branch, unreleased as of 2026-09-09) is the C port of `qc-air.ts` — same ArtifactMask method, same `qc_stats` — so this is the single-implementation path. The segmentation goes in a `mktemp -d` removed by an EXIT trap; brainchop writes the `-o` name literally (verified; `FSLOUTPUTTYPE` is irrelevant to both tools here). `test/browserqc-sh.test.mjs` pins the argv contract and the cleanup on success and failure with mock executables on PATH. Verified on `t1_crop` against `cli/qc.mjs`: every metric within 0.4 % (56/58 within 0.1 %; `summary_wm_p05` 0.37 %, `summary_wm_k` 0.11 %); the residual is Metal-vs-WebGPU label jitter, both sides run the same niimath `--qc` percentile code. **Its JSON is niimath's report, not the browser's**: same metric keys, `size_*`/`spacing_*`, niimath provenance, and no `bids_meta` — a metric-equivalent output, by design (a sidecar merge would need a JSON tool the script deliberately does not depend on).

**`cli/qc.mjs`** — `node cli/qc.mjs --in T1.nii[.gz] --out results.json [--bids sidecar.json]` — headless QC, JSON out. Needs `npm run build` and Chrome.

It **drives the real page in headless Chrome** rather than re-implementing the pipeline in Node, so results are the browser's by construction (verified: every metric bit-identical, and deterministic run-to-run). That is still forced, though the reason changed with the engine: segmentation needs a GPU backend — **WebGPU or WebGL2**, both browser-only APIs Node has no implementation of, so the module refuses in Node. Headless Chrome supplies WebGPU via SwiftShader. (The threaded-CPU backend is no escape hatch: Node has no `crossOriginIsolated`, so the package refuses it there too.)
...
The `@niivue/niimath` 1.3.3 WASM `--qc` has no air term (native niimath's `--air` is the C port of this file), so in the browser the background IQMs are computed here: `summary_bg_*` (8), `snrd_*` (4), `fber`, `qi_1`, and the air term of `cnr` (which replaces niimath's `cnr_noair`). Method mirrors MRIQC's `ArtifactMask`, via four niimath passes in `computeAirQc` ([src/main.ts](src/main.ts)):
...
- **Native JSON is metric-equivalent, not the browser report.** `browserqc.sh` emits niimath's `--json` (niimath provenance, no `bids_meta`); the browser's `buildQcReport` adds BrowserQC provenance + the sidecar. Same metric keys and values, different envelope — documented in README rather than merged (no JSON tool in a two-line shell script).
